...
public class RoundServiceImplementation implements RoundService {
    ...
    public Round createFirstRound(Long tournamentId) throws Exception {
        ...
        // set this round's scoreboard and matchlist, then save round and matches to
        // repository
     ...
     * Given a list of participants sorted by elo, pair up the first half with the
     * second half
     * 
    ...
    public List<Match> createFirstRoundMatches(List<User> participantsList, Round firstRound) {
        ...
        for (int i = 0; i < participantsList.size() / 2; i++) {
            Match match = matchService.createMatch(participantsList.get(i),
                    participantsList.get(participantsList.size() / 2 + i));
     ...
     * Given a list of participants, create a list of scoreboard entries with 0.0
     * score for all participants
     * 
     * @param participantsList list of participants
     * @param firstRound       the first round that the scoreboard entries will be
     *                         in
    ...
    public void checkComplete(Long roundId) throws Exception {
        ...
        // if round is complete, set isCompleted to true and check if tournament is
        // complete
        if (complete) {
            completeRound(round);
        }
     ...
     * Given the tournament, create a new scoreboard that carries over the previous
     * round's scoreboard entries
     * 
     ...
     * Given a new round and its scoreboard entries, pair up the participants by
     * score adjacently
     * 
